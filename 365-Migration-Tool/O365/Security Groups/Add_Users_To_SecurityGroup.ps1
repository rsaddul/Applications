...
$logDir = Join-Path ([Environment]::GetFolderPath("Desktop")) "CloudDonny_Logs\Add_Users_To_SecurityGroup"
...
function Add-ToLog {
    ...
    # Output to GUI launcher (via stderr)
...
"@ | Out-File -FilePath $templatePath -Encoding UTF8 -Force
        ...
        Add-ToLog "💡 You can now edit this file with real group and user emails."
        exit 0
    }
    catch {
...
try {
    ...
    foreach ($row in $rows) {
        ...
        try {
            ...
            $memberEmail = $row.UserPrincipalName

            if ([string]::IsNullOrWhiteSpace($groupEmail) -or [string]::IsNullOrWhiteSpace($memberEmail)) {
                Add-ToLog "⚠️ [$counter/$total] Skipping invalid row (missing GroupEmail or UserPrincipalName)." -Failed
                continue
            }

            Add-ToLog "🔎 [$counter/$total] Processing Group=$groupEmail, User=$memberEmail..."

            # Lookup group by mail
            ...
            $user = Get-MgUser -Filter "UserPrincipalName eq '$memberEmail'" -ErrorAction SilentlyContinue
            if (-not $user) {
                Add-ToLog "❌ User not found: $memberEmail" -Failed
                $failCount++
                continue
            }
            ...
            if ($existingMembers.Id -contains $user.Id) {
                Add-ToLog "⚠️ $memberEmail is already a member of $groupEmail."
                continue
            }

            # Add user to group
            New-MgGroupMember -GroupId $group.Id -DirectoryObjectId $user.Id -ErrorAction Stop
            Add-ToLog "✅ Added $memberEmail to $groupEmail."
            $successCount++
        }
        catch {
            Add-ToLog "❌ Failed to add $($row.UserPrincipalName) to $($row.GroupEmail): $($_.Exception.Message)" -Failed
            $failCount++
        }
    }
