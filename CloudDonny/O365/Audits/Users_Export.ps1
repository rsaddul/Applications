...
$Users = Get-MgUser -All -Property Id,DisplayName,GivenName,Surname,UserPrincipalName,AccountEnabled
...
try {
    ...
    if ($Mailbox.RecipientTypeDetails -eq "SharedMailbox") {
        Add-ToLog "ℹ️ Skipping shared mailbox: $PrimarySMTP"
        continue
    }
...
$SignInStatus = if ($User.AccountEnabled -eq $false) { "Blocked" } else { "Allowed" }
