...
Add-ToLog "🕒 Applying calendar and regional configurations for all mailboxes..."

try {
    $mailboxes = Get-Mailbox -ResultSize Unlimited

    if (-not $mailboxes) {
        Add-ToLog "⚠️ No mailboxes found in tenant." -Failed
        exit 1
    }
    ...
    Add-ToLog "🔹 Summary: $count success(es), $failCount failure(s)."
# ✅ Show popup summary to user
[System.Windows.Forms.MessageBox]::Show(
    "Mailbox timezone and regional settings updated.`n`nSummary:`n$count success(es)`n$failCount failure(s)`n`nLogs saved to:`n$logDir",
    "Operation Complete",
    [System.Windows.Forms.MessageBoxButtons]::OK,
